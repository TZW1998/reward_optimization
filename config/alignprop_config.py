...
def set_config_batch(config,total_samples_per_epoch, total_batch_size, per_gpu_capacity=1):
    ...
    config.train.num_gpus = 8 #len(os.environ["CUDA_VISIBLE_DEVICES"].split(","))
...
def aesthetic():
    ...
    config.reward_fn = 'aesthetic_score' # CLIP or imagenet or .... or ..
    ...
    config = set_config_batch(config, total_samples_per_epoch=128, total_batch_size=32, per_gpu_capacity=2)
    return config
