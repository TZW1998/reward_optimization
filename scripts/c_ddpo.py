...
FLAGS = flags.FLAGS
config_flags.DEFINE_config_file("config", "config/cddpo_config.py", "Training configuration.")
...
def main(_):
    ...
    processed_evaluate_reward_quantile = None
    processed_training_reward_quantile = None
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        for i in tqdm(
            ...
            if processed_evaluate_reward_quantile is None:
                max_reward_cond = None
                use_orig_unet = True
            else:
                max_reward_cond = np.random.choice(processed_evaluate_reward_quantile, size = prompt_embeds.shape[0], replace = True)
                max_reward_cond = torch.as_tensor(max_reward_cond, device = prompt_embeds.device, dtype = prompt_embeds.dtype)
                use_orig_unet = False
        ...
        for i in tqdm(
            ...
            explore_ratio = 2
            explore = torch.rand(1) < explore_ratio

            if (processed_training_reward_quantile is None) or explore:
                max_reward_cond = None
                use_orig_unet = True
                print("use orig unet")
            else:
                max_reward_cond = np.random.choice(processed_training_reward_quantile, size = prompt_embeds.shape[0], replace = True)
                max_reward_cond = torch.as_tensor(max_reward_cond, device = prompt_embeds.device, dtype = prompt_embeds.dtype)
                use_orig_unet = False
                print("use new unet")
        ...
        processed_reward = np.exp(rewards / 20) 

        reward_quantile1 = np.quantile(rewards, 0.8)
        processed_weights= np.zeros_like(rewards)
        # processed_reward[processed_reward < reward_quantile2] = -5
        processed_weights[rewards > reward_quantile1] = 1

        
        processed_evaluate_reward_quantile = np.quantile(processed_reward, q = np.linspace(0.95, 1, 1001))
        if (epoch + 1) % 5 == 0:
            processed_training_reward_quantile = np.quantile(processed_reward, q = np.linspace(0.9, 1, 1001))
        print_processed_training_reward_quantile = None if processed_training_reward_quantile is None else processed_training_reward_quantile[0:1002:100]
        
        print(f"processed_evaluate_reward_quantile: {processed_evaluate_reward_quantile[0:1002:100]}, processed_reward_quantile: {print_processed_training_reward_quantile}")


        samples["processed_reward"] = (
            torch.as_tensor(processed_reward)
            .reshape(accelerator.num_processes, -1)[accelerator.process_index]
            .to(accelerator.device)
        )

        samples["processed_weights"] = (
            torch.as_tensor(processed_weights)
            .reshape(accelerator.num_processes, -1)[accelerator.process_index]
            .to(accelerator.device)
        )
            ...
            for i, sample in tqdm(
                ...
                for j in tqdm(
                    ...
                    with accelerator.accumulate(unet):
                        with autocast():
                            if config.train.cfg:
                                noise_pred = unet(
                                    ...
                                    embeds,
                                    class_labels = torch.cat([sample["processed_reward"]] * 2),
                                ).sample
                        ...
                        #loss = -torch.mean(torch.exp(log_prob))

                        ratio = log_prob # torch.exp(log_prob - sample["log_probs"][:, j])
                        unclipped_loss = - sample["processed_weight"] * ratio
                        # clipped_loss = -advantages * torch.clamp(
                        #     ratio, 1.0 - config.train.clip_range, 1.0 + config.train.clip_range
                        # )
                        # loss = torch.mean(torch.maximum(unclipped_loss, clipped_loss))
        ...
        if (epoch % 10 == 0) and  accelerator.is_main_process:
            ...
            torch.save(pipeline.unet.state_dict(), os.path.join(config.logdir,"c_"+config.run_name + f"_unet_{epoch}.pt"))
