...
FLAGS = flags.FLAGS
config_flags.DEFINE_config_file("config", "config/wddpo_config.py", "Training configuration.")
...
def main(_):
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers(
            project_name="reward_opt-pytorch", config=config.to_dict(), init_kwargs={"wandb": {"name": f"w_{config.run_name}_{config.prompt_fn}_{config.reward_fn}_{config.lora_rank}"}}
        )
    logger.info(f"\n{config}")
    ...
    #processed_reward_quantile = np.zeros(11)
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        for i in tqdm(
            ...
            #max_reward_cond = np.random.choice(processed_reward_quantile, size = prompt_embeds.shape[0], replace = True)
            ...
            for i, sample in tqdm(
                ...
                for j in tqdm(
                    ...
                    with accelerator.accumulate(unet):
                        ...
                        unclipped_loss = - sample["processed_reward"] * ratio
        ...
        if (epoch % 10 == 0) and  accelerator.is_main_process:
            ...
            torch.save(pipeline.unet.state_dict(), os.path.join(config.logdir,"w_"+config.run_name + f"_unet_{epoch}.pt"))
